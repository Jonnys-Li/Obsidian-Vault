...
### 3.2.3 图片展示（运行效果）![[截屏2025-12-19 14.12.34.png]]
![[截屏2025-12-19 14.47.16_副本.png]]
...
- (IBAction)sum {
    //第一步：拿到两个字符串num1 num2
    NSString *sum1String =self.num1.text;
    NSString *sum2String=self.num2.text;
    //加入判断
    /*
    ...
    if (sum2String.length==0) {
     ...
     这种方法只能在服务器上看见输入请求，用户看不见，没啥用
     
     */
    //所以加入提示框让用户看见（UIAlertView）
    /*
    if (sum1String.length==0) {
        NSLog(@"请输入文本");
        //过气方法，但是还能用，可以通过降版本来消除警告
        UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"输入有误" message:@"请输入文本" delegate:nil cancelButtonTitle:@"好的" otherButtonTitles:nil, nil];
        //显示
        [alert show];
    ...
    if (sum2String.length==0) {
        NSLog(@"请输入第一个文本");
        UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"输入有误" message:@"请输入第二个文本" delegate:nil cancelButtonTitle:@"好的" otherButtonTitles:nil, nil];
        //显示
        [alert show];
    }
     这个方法解决了用户看不见的问题，但是代码冗余，所以也不好，要合并一下写成方法
     */
    if (sum1String.length==0) {
        NSLog(@"请输入文本");
        [self showInfo:@"请输入第一个文本"];
    }
    if (sum2String.length==0) {
        
        NSLog(@"请输入文本");
        [self showInfo:@"请输入第二个文本"];
    }
    //第二步：把字符串转换成数值
    ...
    //第三步相加
    NSInteger results=sum1+sum2;
    //显示结果
    self.result.text=[NSString stringWithFormat:@"%zd",results];
    //运行发现即使不输入值也可以运算，这显然不合适，故进行改进
}
//UIView现在已经老得彻底跑不起来了，所以这里创建方法时用的新方法UIAlertController
-(void)showInfo:(NSString *)info {
    // 1. 创建 UIAlertController 对象
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"输入有误"
                                                                             message:info
    ...
    // 3. 显示弹窗（需要由当前的 ViewController 弹出）
...
![[截屏2025-12-19 18.06.22.png]]

## 3.6UI控件类别

![[截屏2025-12-19 18.11.02.png]]
